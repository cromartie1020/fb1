...
def winnerPickListWeek(request):
    '''
    '''
    search = request.GET.get('search')
    if search:
        list=WinnerPick.objects.filter(week_number__exact=search).order_by('-id',)
        p= Paginator(list,16)
        page_number = request.GET.get('page')
        page= request.GET.get('page')
        try:
    ...
    return render(request, 'teams/winnerPickListWeek.html', context)
